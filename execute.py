...
import trade_guard
import risk_guard
import market_regime as market_regime_mod
import market_time
...
def _execute_locked(out, log, force=False):
    ...
    max_new = 0 if regime == "risk_off" else strategy.MAX_NEW_PER_ROUND
    ...
    for decision in sorted(normalized,
        ...
        if decision["action"] == "buy":
            ...
            if decision["code"] not in current_positions:
                if len([p for p in current_positions.values() if p.get("qty", 0) > 0]) >= strategy.MAX_POSITIONS:
                    out(f"  ✗ {decision['code']} 持仓已达上限 {strategy.MAX_POSITIONS}，拒绝买入")
                    continue
            ...
            max_value = float(equity) * strategy.MAX_WEIGHT
