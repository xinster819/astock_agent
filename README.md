...
astock dashboard              # 生成对照实验观察台 reports/dashboard.html
...
astock/
  ...
  reporting/   只读产出层，绝不写账本
    ...
    analytics.py  ★   绩效统计与「能不能下结论」的判据（纯函数）
    console.py    ★   观察台装配 + 单文件 HTML 渲染
    webapp/           观察台前端：index.html / console.css / console.js
...
```bash
astock dashboard              # 生成 reports/dashboard.html，双击打开
astock dashboard --offline    # 不拉实时行情与基准指数
```

单文件 HTML，无外部依赖，断网也能看，可以直接发给别人。
带实时行情时约需 2~3 分钟（持仓逐只走三源交叉验证，实测单只约 10s），
命令行会打进度；只想快速看一眼就加 `--offline`。
**不起服务、不开端口**——项目的硬约束之一是「禁止启动网络监听进程」，
而观察台是只读的、数据源本就是定时任务产出的 CSV，没有任何需要实时推送的东西。

### 它的主张：不让你过早下结论

排行榜是危险的东西——第一名这个位置本身就是一种结论。而实测数据长这样：
...
按收益排，D 组第一。但它**一笔都没平过仓**（那 +1.55% 全是浮盈），
观察期还只有 exp5 的一半。任何普通看板都会让人读出「新闻情绪策略胜出」——
这个结论毫无支撑。

所以观察台把**「能不能比」放在「谁在赢」前面**：

- 顶部是**判据条**，先回答三关：闸门、样本量、观察期是否可比
- 排行榜里**样本达标的账户永远排在前面**，中间一行明说下面这些为什么不参与排名
- 每个账户标注样本分级（`样本不足` / `仅供观察` / `可初步比较`）
- 归因面板给出**统计判定**：单笔期望是否显著不为零。小样本一律**不判**，
  并用自由度对应的 t 临界值而非固定的 2——df=2 时临界值是 4.30，
  拿 2 去判会把「3 笔全赢」认证成有优势
- 「从未平仓」的账户单独点名：它们的收益全是浮盈，不构成业绩

所有结论都在 Python 侧算完再送给前端，前端只负责画。
这样每一个会被人当作结论的数字都有单元测试盯着，而不是藏在浏览器里。
...
|---|---|
| 判据条 | 现在能不能下结论 |
| 闸门条 | 引擎停摆了吗？账本对得上吗？市场状态如何 |
| 排行榜 | 谁在赢、赢多少（可按层筛选、按任意列排序） |
| 权益曲线 | 三种口径：累计收益 / 归一化 / 回撤，可叠加沪深300 基准 |
| 基准对齐 | 指数**裁剪到账户观察期**再归一化——拿「账户自 7 月 +1%」比「指数自 3 月 +5%」没有意义。实测同期沪深300 −7.61%，而多数账户在 −1%~0%，这层背景光看收益率读不出来 |
| 归因面板 | 赢在哪：胜率、盈亏比、单笔期望、最大回撤、仓位集中度、持仓与流水 |
| 持仓重叠 | **这些「独立实验」真的独立吗**——同一只票被多组持有时，收益会同涨同跌 |
